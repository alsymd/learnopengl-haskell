...
actuate :: VertexArrayObject -> VertexArrayObject -> Program -> Program ->  UniformFunc (GLmatrix GLfloat) -> UniformFunc (GLmatrix GLfloat) -> UniformFunc (GLmatrix GLfloat) -> UniformFunc (Vector3 GLfloat) -> Window -> Bool -> (Float, Maybe OutputEvent, ObjectState) -> IO Bool
actuate cubeVao lightVao cubeProgram lightProgram uM uPVM uLPVM uViewPos  window _ (t, oe, ObjectState {pos = position, direct = direction}) =
  if oe == Just QuitG
    then pure True
    else do
...
main
 ...
 = do
  ...
  -- Load shaders
  vs <- loadShaderFromFile VertexShader "pointLight.vert"
  lvs <- loadShaderFromFile VertexShader "pointLightLight.vert"
  fs <- loadShaderFromFile FragmentShader "pointLight.frag"
  lfs <- loadShaderFromFile FragmentShader "pointLightLight.frag"
  ...
  Y.reactimate Main.initialize (sense timeRef) (actuate cubevao lightvao  cubeProgram lightProgram uM uPVM uLPVM uViewPos window) sig
