...
modelLight = set translation (V3 1.2 1 2) Linear.Matrix.identity !*! scale 0.2 0.2 0.2
...
actuate :: VertexArrayObject -> VertexArrayObject -> Program -> Program ->  UniformFunc (GLmatrix GLfloat) -> UniformFunc (GLmatrix GLfloat) -> UniformFunc (GLmatrix GLfloat) -> UniformFunc (Vector3 GLfloat) ->  UniformFunc (Vector3 GLfloat) -> UniformFunc (Vector3 GLfloat) -> Window -> Bool -> (Float, Maybe OutputEvent, ObjectState) -> IO Bool
actuate cubeVao lightVao cubeProgram lightProgram uM uPVM uLPVM uViewPos uLDirection uLPosition   window _ (t, oe, ObjectState {pos = position, direct = direction}) =
  if oe == Just QuitG
    then pure True
    else do
    ...
    uLPosition $ Vector3 x y z
    let (x',y',z') = Y.vector3XYZ direction in (uLDirection $ Vector3 x' y' z')
...
createTextureFromFile filePath = do
  Right di <- readImage filePath
  let img = flipVertically . convertRGB8 $ di
...
main
 ...
 = do
  ...
  -- Load shaders
  vs <- loadShaderFromFile VertexShader "multipleLights.vert"
  lvs <- loadShaderFromFile VertexShader "multipleLightsLight.vert"
  fs <- loadShaderFromFile FragmentShader "multipleLights.frag"
  lfs <- loadShaderFromFile FragmentShader "multipleLightsLight.frag"
  ...
  uLPVM <- uniformFunc lightProgram "pvm" :: IO (UniformFunc (GLmatrix GLfloat))
  ...
  uLAmbient <- uniformFunc cubeProgram "spotLights[0].ambient" :: IO (UniformFunc (Color3 GLfloat))
  uLDiffuse <- uniformFunc cubeProgram  "spotLights[0].diffuse" :: IO (UniformFunc (Color3 GLfloat))
  uLSpecular <- uniformFunc cubeProgram  "spotLights[0].specular" :: IO (UniformFunc (Color3 GLfloat))
  uLConstant <- uniformFunc cubeProgram "spotLights[0].constant" :: IO (UniformFunc GLfloat)
  uLLinear <- uniformFunc cubeProgram "spotLights[0].linear" :: IO (UniformFunc GLfloat)
  uLQuadratic <- uniformFunc cubeProgram "spotLights[0].quadratic" :: IO (UniformFunc GLfloat)
  uLPosition <- uniformFunc cubeProgram "spotLights[0].position" :: IO (UniformFunc (Vector3 GLfloat))
  uLCutOff <- uniformFunc cubeProgram "spotLights[0].cutOff" :: IO (UniformFunc GLfloat)
  uLDirection <- uniformFunc cubeProgram "spotLights[0].direction" :: IO (UniformFunc (Vector3 GLfloat))
  uLOuterCutOff <- uniformFunc cubeProgram "spotLights[0].outerCutOff" :: IO (UniformFunc GLfloat)
  ...
  uLCutOff $ cos 0.218
  uLOuterCutOff $ cos 0.30533


  -- Set clearColor to something darker for better contrast
  clearColor $= Color4 0.01 0.01 0.01 1
  timeRef <- newIORef =<< getCurrentTime
  Y.reactimate Main.initialize (sense timeRef) (actuate cubevao lightvao  cubeProgram lightProgram uM uPVM uLPVM uViewPos  uLDirection uLPosition window) sig
