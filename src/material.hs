...
actuate :: VertexArrayObject -> VertexArrayObject -> Program-> Program -> UniformLocation -> UniformLocation -> UniformLocation -> UniformLocation ->UniformLocation -> UniformLocation -> UniformLocation -> UniformLocation -> UniformLocation ->UniformLocation -> UniformLocation -> Window -> Bool -> (Float, Maybe OutputEvent, ObjectState) -> IO Bool
actuate cubeVao lightVao cubeProgram lightProgram locLAmbient locLDiffuse locLSpecular locAmbient locDiffuse locSpecular locShininess locPL locP locC locL window _ (t, oe, ObjectState {pos = position, direct = direction}) =
  if oe == Just QuitG
    then pure True
    else do
    ...
    uniform locLAmbient $= (Vector3 1 1 1 :: Vector3 GLfloat)
    uniform locLDiffuse $= (Vector3 1 1 1 :: Vector3 GLfloat)
    uniform locLSpecular $= (Vector3 1 1 1 :: Vector3 GLfloat)
    uniform locPL $= (Vector3 lightx lighty lightz :: Vector3 GLfloat)
    uniform locAmbient $= (Vector3 0 0.1 0.06 :: Vector3 GLfloat)
    uniform locDiffuse $= (Vector3 0 0.50980392 0.50980392 :: Vector3 GLfloat)
    uniform locSpecular $= (Vector3 0.50196078 0.50196078 0.50196078 :: Vector3 GLfloat)
 ...
 = do
  ...
  vertexAttribPointer (AttribLocation 1) $=
    ...
    , VertexArrayDescriptor
        ...
        Float
        (fromIntegral $ 6 * sizeOf (undefined :: GLfloat))
        (intPtrToPtr  .fromIntegral $3 * sizeOf (undefined ::GLfloat)))  -- Enable depth buffer
  vertexAttribArray (AttribLocation 0) $= Enabled
  ...
  -- Load shaders
  vs <- loadShaderFromFile VertexShader "material.vert"
  fs <- loadShaderFromFile FragmentShader "material.frag"
  ...
  loc6 <- get . uniformLocation cubeProgram $ "material.ambient"
  ...
  -- uniform loc4 $= (Vector3 1.2 1 2 :: Vector3 GLfloat)


  -- Set clearColor to something darker for better contrast
  clearColor $= Color4 0.1 0.1 0.1 1
  timeRef <- newIORef =<< getCurrentTime
  Y.reactimate Main.initialize (sense timeRef) (actuate cubevao lightvao cubeProgram lightProgram loc10 loc11 loc12 loc6 loc7 loc8 loc9 loc4 loc5 loc2 loc3 window) sig
