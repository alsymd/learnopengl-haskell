...
actuate :: VertexArrayObject -> VertexArrayObject -> Program-> Program -> UniformLocation -> UniformLocation ->UniformLocation -> UniformLocation -> Window -> Bool -> (Float, Maybe OutputEvent, ObjectState) -> IO Bool
actuate cubeVao lightVao cubeProgram lightProgram locPL locP locC locL window _ (t, oe, ObjectState {pos = position, direct = direction}) =
  if oe == Just QuitG
    then pure True
    else do
...
glConfig = defaultOpenGL {glProfile = profile, glMultisampleSamples = 2}
...
main
 ...
 = do
  ...
  -- Load shaders
  vs <- loadShaderFromFile VertexShader "specular.vert"
  fs <- loadShaderFromFile FragmentShader "specular.frag"
  lfs <- loadShaderFromFile FragmentShader "specularLight.frag"
  ...
  loc4 <- get . uniformLocation cubeProgram $ "lightPos"
  ...
  Y.reactimate Main.initialize (sense timeRef) (actuate cubevao lightvao cubeProgram lightProgram loc4 loc5 loc2 loc3 window) sig
