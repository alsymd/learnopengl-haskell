...
main
 ...
 = do
  ...
  -- Load shaders
  vs <- loadShaderFromFile VertexShader "spotLight.vert"
  lvs <- loadShaderFromFile VertexShader "spotLightLight.vert"
  fs <- loadShaderFromFile FragmentShader "spotLight.frag"
  lfs <- loadShaderFromFile FragmentShader "spotLightLight.frag"
  ...
  uLCutOff <- uniformFunc cubeProgram "light.cutOff" :: IO (UniformFunc GLfloat)
  uLDirection <- uniformFunc cubeProgram "light.direction" :: IO (UniformFunc (Vector3 GLfloat))
  uLOuterCutOff <- uniformFunc cubeProgram "light.outerCutOff" :: IO (UniformFunc GLfloat)
