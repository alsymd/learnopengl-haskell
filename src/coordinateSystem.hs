...
actuate loc window _ (t, s) =
  if s == Quit
    then pure True
         ...
         ((toMatrix (proj!*!(trans t)) :: IO (GLmatrix GLfloat)) >>= (uniform loc$=))*>
...
trans angle = let r = axisAngle (V3 1 0 0) angle
                  t = V3 0 0 (-3)
...
main
 ...
 = do
  ...
  -- Load shaders
  vs <- loadShaderFromFile VertexShader "coordinateSystem.vert"
  fs <- loadShaderFromFile FragmentShader "coordinateSystem.frag"
